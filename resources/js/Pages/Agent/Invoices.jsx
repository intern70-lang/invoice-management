...
export default function Invoices({ invoices = [] }) {
    const handleDelete = (invoice) => {
        router.delete(`/agent/invoices/${invoice.id}`, {
            preserveScroll: true,
        });
    };
    ...
    const columns = [
        {
            ...
            key: "invoice_number",
            render: (num) => <Text strong className="text-(--primary)">{num}</Text>,
        },
        {
            title: "Customer",
            key: "customer",
            render: (_, record) => <Text strong>{record.customer?.name ?? "—"}</Text>,
        },
        {
            title: "Date",
        ...
        },
        {
            ...
            align: "right",
            render: (val) => (
                <Text type="secondary">£{Number(val ?? 0).toFixed(2)}</Text>
            ),
        },
        {
            title: "Total",
            ...
            render: (_, record) => (
                <Space size="small">
                    <Tooltip title="View Invoice">
                        <Link href={`/agent/invoices/${record.id}`}>
                ...
                <div className="space-y-5">
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-sm font-semibold text-(--text-primary) m-0!">Invoices</h2>
                        ...
                        <Link href="/agent/invoices/create">
