...
export default function CreateInvoice({ customers, products, areas = [], settings }) {
    ...
    useEffect(() => {
        // Admin has next-number endpoint; Agent uses same generator in backend.
        // If you later create a dedicated agent endpoint, update this URL.
        fetch("/admin/invoices/next-number")
            .then((r) => r.json())
            .then(({ number }) => setInvoiceNumber(number))
            .catch(() => setInvoiceNumber(null));
    }, []);
    ...
    const loadCustomerInfo = async (id) => {
        ...
        try {
            const res = await fetch(`/agent/customers/${id}/data`);
    ...
    const handleSubmit = async () => {
        ...
        const invalidRow = rows.find((r) => !r.product_id || r.qty < 1 || r.selling_price < 0);
        if (invalidRow) {
            setErrors({ items: "Please select a product and enter valid qty/price for all rows." });
            return;
        }
        ...
        router.post("/agent/invoices", payload, {
            preserveScroll: true,
            onError: (errs) => {
                setSubmitting(false);
            ...
            <AppLayout title="Create Invoice">
                <div className="max-w-5xl mx-auto flex items-center gap-3 mb-4">
                    <Link href="/agent/invoices">
                        <Button icon={<ArrowLeft size={15} />} size="small" color="default" variant="filled" />
                    </Link>
                    <h2 className="text-sm font-semibold text-(--text-primary) m-0!">New Invoice</h2>
                </div>
                ...
                <div className="max-w-5xl mx-auto space-y-5 bg-(--bg-secondary) p-3 rounded-2xl border border-(--border-color)">
                    ...
                    >
                        <div className="card p-5 mb-4 space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                >
                                    <DatePicker
                                        ...
                                        onChange={() => {
                                            ...
                                            const inv = form.getFieldValue("invoice_date");
                                            if (due && inv && due.isBefore(inv, "day")) {
                                ...
                                <Form.Item
                                    ...
                                    rules={[
                                        ...
                                        ({ getFieldValue }) => ({
                                            validator(_, value) {
                                                const inv = getFieldValue("invoice_date");
                                                if (!value || !inv) return Promise.resolve();
                                                if (value.isBefore(inv, "day")) {
                                                    return Promise.reject("Due date must be on or after invoice date");
                                                }
                                ...
                                >
                                    <DatePicker
                                        ...
                                        disabledDate={(d) => {
                                            const inv = form.getFieldValue("invoice_date");
                                            return inv ? d && d.isBefore(inv, "day") : false;
                                        }}
                                    />
                                </Form.Item>
                                    ...
                                    {[
                                        ...
                                        { label: "Phone", value: customerInfo.phone },
                                        {
                                            ...
                                            value: customerInfo.birthdate
                                                ? new Date(customerInfo.birthdate).toLocaleDateString("en-US", {
                                                      month: "2-digit",
                                                      day: "2-digit",
                                                      year: "numeric",
                                                  })
                                        ...
                                        { label: "Shipping Address", value: customerInfo.shipping_address },
                                        ...
                                        { label: "VAT Registered", value: customerInfo.vat_registered ? "Yes" : "No" },
                                        ...
                                        <div key={label}>
                                            ...
                                            <p className="text-sm text-(--text-primary) mt-0.5 m-0! truncate">{value || "—"}</p>
                                        </div>
                                    ))}
                                </div>
                            )}

                            <Form.Item label="Remarks / Notes" name="remarks" className="m-0!">
                                <TextArea rows={3} placeholder="Add payment terms, banking details or notes..." className="resize-none text-sm" />
                            </Form.Item>

                            <Divider className="my-4!" />

                            <div className="flex items-center justify-between mb-3">
                                <Text strong className="text-sm">Invoice Items</Text>
                                <Button icon={<Plus size={14} />} size="small" onClick={addRow}>
                                    Add Product
                                </Button>
                            </div>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-gray-400/30">
                                            {[
                                                ...
                                                { label: "Price (ex. VAT)", align: "right" },
                                                { label: "VAT %", align: "center" },
                                                { label: "Qty", align: "right" },
                                                { label: "VAT Amt", align: "right" },
                                    ...
                                    <tbody>
                                        {rows.map((row) => (
                                            <tr key={row._key} className="border-b border-gray-400/30">
                                                <td className="py-2 pr-2 min-w-[180px]">
                                                    <Select
                                                        ...
                                                        onChange={(val) => onProductChange(row._key, val)}
                                                        className="w-full"
                                                        status={errors.items && !row.product_id ? "error" : undefined}
                                                    />
                                                </td>
                                                ...
                                                <td className="py-2 px-2 w-32">
                                                    <InputNumber
                                                        ...
                                                        onChange={(val) => updateRow(row._key, { selling_price: val ?? 0 })}
                                                        className="w-full"
                                                    />
                                                </td>
                                                ...
                                                <td className="py-2 px-2 w-24">
                                                    <Select
                                                        ...
                                                        onChange={(val) => updateRow(row._key, { vat_percent: val })}
                                                        className="w-full"
                                                    />
                                                </td>
                                                ...
                                                <td className="py-2 px-2 w-20">
                                                    <InputNumber
                                                        size="middle"
                                                        min={(products ?? []).find((p) => p.id === row.product_id)?.moq ?? 1}
                                                        max={(products ?? []).find((p) => p.id === row.product_id)?.qty ?? 999999}
                                                        precision={0}
                                                        value={row.qty}
                                                        onChange={(val) => updateRow(row._key, { qty: val ?? 1 })}
                                                        className="w-full"
                                                        status={errors.items && row.qty < 1 ? "error" : undefined}
                                                    />
                                                </td>

                                                <td className="py-2 px-2 text-right text-(--text-secondary) whitespace-nowrap">{fmt(currency, row.vatAmt)}</td>
                                                <td className="py-2 px-2 text-right font-medium text-(--text-primary) whitespace-nowrap">{fmt(currency, row.lineTotal)}</td>
                                        ...
                                        {rows.length === 0 && (
                                            <tr>
                                                <td colSpan={7} className="py-6 text-center text-(--text-secondary) text-xs">
                            ...
                            <div className="flex justify-end pt-4">
                                <div className="w-64 space-y-2">
                                    <div className="flex justify-between text-sm">
                                        <Text type="secondary">Total VAT (exclusive)</Text>
                                    ...
                                    <div className="flex justify-between text-sm font-semibold border-t border-gray-400/30 pt-2">
                                        ...
                                        <Text strong className="text-(--primary)">{fmt(currency, totalAmount)}</Text>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <Button type="primary" icon={<Save size={14} />} loading={submitting} onClick={handleSubmit}>
                                Save Invoice
                            </Button>
                        </div>
                    </Form>
